 ...
 *  是notebook的内存对象，zeppelin管理 比如 持久化，或者共享的最小单元，很多功能与管理Paragraph有关，
...
public class Note implements Serializable, ParagraphJobListener {
  ...
  public void run(String paragraphId) {
    /*
    根据paragraphId得到想对应的p，里面存放了该paragraph内容
     */
...
/*
如果paragraph为空，即没写代码，日志打印出skip to run blank paragraph. {}，并且输出paragraphId
设置此paragraph的状态为finished，程序中止
 */
...
/*
如果不为空，获得用户引用的%sql解析器
 */
    String requiredReplName = p.getRequiredReplName();
    /*
    从factory中获得这个解析器的具体相关配置和信息参数
     */
    Interpreter intp = factory.getInterpreter(p.getUser(), getId(), requiredReplName);
/*
如果这个解析器不存在的话，
赋值intpException：报出解释器没有找到的异常p.getJobName() + "'s Interpreter " + requiredReplName + " not found"
更新intpResult将 结果为ERRO，并且将异常信息存到结果中
设置状态为ERRO，跑出intpException
 */
    if (intp == null) {
    ...
    /*
    如果可以得到的配置文件可以用，pragraph进行身份设置，在以后验证中用
    intp这个Interpreter对象获得Scheduler，创建FIFO调度，在初始化调度时，初始化他的name，excutor，和listener
    可以控制job的中止，得到job目前的状态是waiting还是running等
    将这个任务提交到job队列
    submit是Scheduler的抽象方法，由于是创建了FIFOSchedule的job调度，实现了这个接口，我们去这里面继续寻找这个方法的具体实现
    FIFOScheduler.submit方法，
