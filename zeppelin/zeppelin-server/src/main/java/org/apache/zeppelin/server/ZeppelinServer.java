 ...
 * 整个系统入口，提供服务器功能、权限认证以及用户校验等功能。该模块中有个ZeppelinServer类，直接run
 * 1：内嵌jetty服务器，支持以WebSocket和ReST两种方式对外暴露系统功能
 * 2：创建NotebookServer实例，建立起WebSocket Connection和消息处理的服务端
 * 3：创建Notebook需要的相关依赖，如Note持久化服务（NotebookPro），note的全文索引服务（SearchService），并完成向Note，Paragraph的注入
...
public class ZeppelinServer extends Application {
  ...
  private SchedulerFactory schedulerFactory;//调度
  private InterpreterFactory replFactory;//具体实现类
  private SearchService noteSearchService;//查找
  private NotebookRepoSync notebookRepo;//持久化
  private NotebookAuthorization notebookAuthorization;
  private Credentials credentials;//身份验证
...
/**
 * DependencyResolver：
 *初始化动态依赖加载器，mvc框架的一个组建，负责依赖对象的解析，很多内部对象的创建都是通过它完成的
 */
...
/**
 * Helium 是一个前端响应式 Web 框架，用于使用 HTML5 + CSS3 快速制作原型以及开发产品
 */
