 ...
 * 将Notebook，Note,Paragraph。Interpreter等类封装，通过WebSocket的形式对web客户端提供出去
 * 1：维护Websocket连接与Note之间的映射关系
 * 2：处理客户端和服务器之间的双向通信（通过WebSocket，具体的通信协议见Message类），
 *    包括消息的序列化反序列化，消息解析和服务端处理，处理结果的向客户端广播/单播发送等
 * 3：Note的CRUD操作，以及Paragraph的CRUD操作，执行导入，导出的权限控制
